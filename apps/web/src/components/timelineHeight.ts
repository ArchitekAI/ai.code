...
const USER_TEXT_ATTACHMENT_CARD_HEIGHT_PX = 88;
...
interface TimelineMessageHeightInput {
  ...
  text: string;
  attachments?: ReadonlyArray<{ id: string; type?: "image" | "text" }>;
}
...
export function estimateTimelineMessageHeight(
  ...
  if (message.role === "user") {
    ...
    const imageAttachmentCount =
      message.attachments?.filter((attachment) => attachment.type !== "text").length ?? 0;
    const textAttachmentCount =
      message.attachments?.filter((attachment) => attachment.type === "text").length ?? 0;
    const attachmentRows = Math.ceil(imageAttachmentCount / ATTACHMENTS_PER_ROW);
    const attachmentHeight =
      attachmentRows * USER_ATTACHMENT_ROW_HEIGHT_PX +
      textAttachmentCount * USER_TEXT_ATTACHMENT_CARD_HEIGHT_PX;
