...
import { linearThreadKeyFor, normalizeLinearWebhookInput } from "../src/linear/ingress.ts";
...
export const upsertFromLinearIngress = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existingMessage = await ctx.db
      ...
      .withIndex("by_external_message_key", (query: any) => query.eq("externalMessageKey", args.eventId))
      .unique();
...
export const listControlThreads = internalQuery({
  ...
  returns: v.array(
    v.object({
      ...
      threadKey: v.string(),
      threadKind: v.union(v.literal("issue"), v.literal("comment")),
      state: v.union(v.literal("open"), v.literal("active"), v.literal("waiting"), v.literal("closed")),
